...
def Basic_LOBPCG_standard(A, X0, nev,
...
def BLOPEX_LOBPCG_standard(A, X0, nev,
                           T=None, itmax=200, tol=1e-6,
                           A_products='implicit'):
  """
...
def Ortho_LOBPCG_standard(A, X0, nev,
                          T=None, itmax=200, tol=1e-6,
                          A_products='implicit'):
  """
...
def Skip_ortho_LOBPCG_standard(A, X0, nev,
                               T=None, itmax=200, tol=1e-6,
                               A_products='implicit'):
  """
  ...
  if k < nev:
    for j in range(1, itmax + 1):
      ...
      if j == 1:
        if skip_ortho:
          AZ[:] = A @ Z
          hX, Lambda, skip_ortho, VtV = RR2(X, Z, AX, AZ, Z, True)
          hX_X, hX_Z = hX[:m, :], hX[m:2*m, :]
          if skip_ortho:
            hY = vstack([zeros((m, m)), hX_Z])
            norm_VtVXZ = norm(VtV @ vstack([hX_X, hX_Z]))
            OrthoB(hY, VtV, vstack([hX_X, hX_Z]), VtV @ hY, norm_VtVXZ)
            hY_X, hY_Z = hY[:m, :], hY[m:2*m, :]
            if A_products == 'implicit':
              # AP[:] = AX @ hY_X + AZ @ hY_Z
              matmul(AZ, hY_Z, out=AP)
        ...
        if skip_ortho:
          AZ[:] = A @ Z
          hX, Lambda, skip_ortho, VtV = RR3(X, Z, P, AX, AZ, AP, Z, P, True)
          hX_X, hX_Z, hX_P = hX[:m, :], hX[m:2*m, :], hX[2*m:3*m, :]
          if skip_ortho:
            hY = vstack([zeros((m, m)), hX_Z, hX_P])
            norm_VtVXZP = norm(VtV @ vstack([hX_X, hX_Z, hX_P]))
            OrthoB(hY, VtV, vstack([hX_X, hX_Z, hX_P]), VtV @ hY, norm_VtVXZP)
            hY_X, hY_Z, hY_P = hY[:m, :], hY[m:2*m, :], hY[2*m:3*m, :]
            if A_products == 'implicit':
              # W[:] = AX @ hY_X + AZ @ hY_Z + AP @ hY_P
              matmul(AP, hY_P, out=W)
